...
function ApplicantTable({ applications = [], onUpdateStatus, onDownloadResume }) {
  ...
  const [viewingCandidateResume, setViewingCandidateResume] = useState(null);
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-100">
              {selectedCandidate.resume_file_name ? (
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    type="button"
                    onClick={() => setViewingCandidateResume(selectedCandidate)}
                    className="inline-flex items-center gap-2 rounded-full bg-indigo-50 border border-indigo-200 px-4 py-2 text-xs font-bold text-indigo-950 shadow-sm hover:bg-indigo-100 transition"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-3.5 py-2 text-xs font-bold text-slate-900 shadow-sm hover:bg-slate-50 transition"
                  >
      ...
      <ResumeViewerModal
        open={Boolean(viewingCandidateResume)}
        onClose={() => setViewingCandidateResume(null)}
        candidate={viewingCandidateResume}
        onUpdateStatus={async (appId, newStatus) => {
          await handleStatusChange(appId, newStatus);
          setViewingCandidateResume((prev) => (prev ? { ...prev, status: newStatus } : null));
