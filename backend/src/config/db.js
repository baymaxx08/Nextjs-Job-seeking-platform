  ...
  async connect() {
    if (realPool) {
      try {
        ...
        console.warn('[AI Studio] PostgreSQL connection failed, switching to in-memory store:', err.message);
      }
    }
  ...
  async query(text, params) {
    if (realPool) {
      try {
        ...
        console.warn('[AI Studio] PostgreSQL query failed, executing with in-memory store:', err.message);
      }
    }
    return executeMockQuery(text, params);
  },
